...
Script location: `scripts/start.sh` (**implemented**). It starts **MagicBall → Data Scout → Intelligence Layer → Notification Gateway → UI** (ports 4100–4104) in that order, health-gated: services use `GET /health`; the UI uses **`GET /api/health`** on port `4104`. Set `SKIP_INTELLIGENCE=1` to run only MagicBall and Data Scout (e.g. when MongoDB/Gemini are not available). Set `SKIP_NOTIFICATION_GATEWAY=1` after Intelligence Layer to skip the gateway (IL-only local). Set **`SKIP_UI=1`** to stop after the Notification Gateway (backend stack only, no Next.js). **Prerequisites:** local Redis, MongoDB, a valid `GEMINI_API_KEY` for Intelligence Layer AI steps, a seeded Notification Gateway user directory (`pnpm --filter notification-gateway seed`) before relevance/delivery end-to-end runs, and (for the full stack including UI) `pnpm --filter ui dev` or a completed `dev:stack` run with UI not skipped.
